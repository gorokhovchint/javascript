...
//создал пустой массив некое подобие gtm, чтобы показать что dataLayer это просто название, далее при совершении события которое добавил в обработчик,
//меняется цвет и методом push добавляется событие в формате ключ:значение т.е создался ассоциативный массив .
let dataLayer23 = []
...
// TASK DOM - стили можно присвоить 3 способами(1:атрибут=value в html документ, 2:таблица стилей в html документе, 3: таблица стилей в css документе, 
// А  в документе js управлять стилями)
...
aModel.push(197)// пишем переменную, добавляем метод push и в скобках указываем значения которые хотим добавить. 
//Отсюда исходит расшифровка в gtm переменной datalayer - это всего лишь название переменной, где гугл при разработке ее создал и в интерфейса gtm написал переменная уровня данных. 
...
for ( let i = 0; i < cBrend.length; i++) {
  ...
  // 0 < cBrend[0].length у нулевого индекса 4 элемента т.е 0<4 - ДА, условие сработало и переменную k выводим с перменной cBrend[0][k] у которой нулевой индекс [0], 
//на выходе мы получаем cBrend[0][0] - и цикл выводит нам это значение. И дальше переменная k увел + 1 и  переберает следующие индексы т.е Brend[0][1], Brend[0][2] 
//и т.д со всеми остальными [i] - их 3. [0][4],[1][3],[2][3]- нужно перебрать 10 элементов. 
//END TASK
...
//Про метод GET - мы делаем запрос на сервер, где передаем get параметры в URL например: /?content=img или просто можем запросить адрес страницы например: /users 
//т.е мы можем просто передать какие-то значения на сервер типо utm_меток и в ответ ни чего не получить, кроме страницы, а может как на пример с /users или /?content=img 
//получить ответ от сервера, в формате json документа или html документ страницы который был сформирован по конкретным get параметрам. 
